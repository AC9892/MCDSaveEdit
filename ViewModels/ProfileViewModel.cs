...
namespace MCDSaveEdit.ViewModels
{
    public class ProfileViewModel
    {
        ...
        public ProfileViewModel()
        {
            ...
            mainEquipmentModel.onChanged = notifyChanged;
            storageChestEquipmentModel.onChanged = notifyChanged;
        ...
        {
            if (_profile.value?.Items == null || !_profile.value.Items.Contains(item)) return false;

            mainEquipmentModel.selectEnchantment(null);
            mainEquipmentModel.selectItem(null);
            mainEquipmentModel.removeItem(item);

            // Items in storage must not remain equipped. Clear this only after
            // removal so the equipped-item observers are refreshed correctly.
            item.EquipmentSlot = null;
            storageChestEquipmentModel.addItemToList(item);
            storageChestEquipmentModel.selectItem(item);
            return true;
        }

        public bool moveItemToInventory(Item item)
        {
            if (_profile.value?.StorageChestItems == null || !_profile.value.StorageChestItems.Contains(item)) return false;

            storageChestEquipmentModel.selectEnchantment(null);
            storageChestEquipmentModel.selectItem(null);
            storageChestEquipmentModel.removeItem(item);

            item.EquipmentSlot = null;
            mainEquipmentModel.addItemToList(item);
            mainEquipmentModel.selectItem(item);
            return true;
        }
