...
namespace MCDSaveEdit.Services
{
    public class PakContentResolver : IImageResolver, ILanguageResolver
    {
        ...
        private readonly HashSet<string> _failedBitmapPaths = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        ...
        public int cachedImageCount { get { lock (_bitmaps) return _bitmaps.Count; } }
        ...
        {
            return Task.Run(() => loadPakFiles(preloadBitmaps));
        }
        ...
        public BitmapImage? imageSource(string pathWithoutExtension)
        {
            ...
            lock (_bitmaps)
            {
                if (_bitmaps.TryGetValue(path, out BitmapImage cached)) return cached;
                if (_failedBitmapPaths.Contains(path)) return _backupResolver.imageSource(path);
            }

            try
            {
                BitmapImage? bitmap = _pakIndex.extractBitmap(path);
                if (bitmap == null)
                {
                    rememberImageFailure(path, "The asset was not present or could not be decoded.");
                    return _backupResolver.imageSource(path);
                }
                if (bitmap.CanFreeze) bitmap.Freeze();
                lock (_bitmaps) _bitmaps[path] = bitmap;
                return bitmap;
            }
            catch (Exception e)
            {
                rememberImageFailure(path, e.Message);
                return _backupResolver.imageSource(path);
            }
        }
        ...
        public void clearImageCache()
        {
            ...
            {
                _bitmaps.Clear();
                _failedBitmapPaths.Clear();
            }
            _backupResolver.clearImageCache();
            lastImageError = null;
        }

        private void rememberImageFailure(string path, string reason)
        {
            lock (_bitmaps) _failedBitmapPaths.Add(path);
            lastImageError = $"Could not load game image '{Path.GetFileName(path)}': {reason}";
            EventLogger.logError(lastImageError);
        }
