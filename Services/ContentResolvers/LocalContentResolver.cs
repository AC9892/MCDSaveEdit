...
{
    public class LocalContentResolver: IImageResolver, ILanguageResolver
    {
        ...
        private readonly Dictionary<string, BitmapImage> _bitmaps = new Dictionary<string, BitmapImage>(StringComparer.OrdinalIgnoreCase);
        ...
        {
            return tryBitmapImageForUri(new Uri(Path.Combine(IMAGES_URI, "Unknown.png")));
        }

        public void clearImageCache()
        {
            lock (_bitmaps) _bitmaps.Clear();
            lastImageError = null;
        }
        ...
        private BitmapImage? tryBitmapImageForUri(Uri uri)
        {
            lock (_bitmaps)
            {
                if (_bitmaps.TryGetValue(uri.OriginalString, out BitmapImage cached)) return cached;
            }
            try
            {
                ...
                lock (_bitmaps) _bitmaps[uri.OriginalString] = image;
                return image;
            }
            catch (Exception e)
            {
                lastImageError = $"Could not load bundled image '{Path.GetFileName(uri.LocalPath)}': {e.Message}";
        ...
        {
            if (ItemDatabase.artifacts.Contains(type))
            {
                return "Artifacts";
            }
            if (ItemDatabase.armor.Contains(type))
            {
                return "Armor";
            }
            if (ItemDatabase.meleeWeapons.Contains(type))
            {
                return "MeleeWeapons";
            }
            if (ItemDatabase.rangedWeapons.Contains(type))
            {
